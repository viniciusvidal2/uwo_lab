...
int main(int argc, char **argv)
{
  ...
  string robot_name, processor_name;
  n_.param<string>("robot_name", robot_name, "robot");
  n_.param<string>("processor", processor_name, "fog");
  float pct_cpu_idle = 15.0;

  string cpu_topic_name = "/"+robot_name+"/cpu_monitor/total_cpu";
  ...
  while(ros::ok()){
    ...
    if(current_total_cpu <= pct_cpu_idle && pct_cpu_idle > 0 && cpus.size() > 15){
      ...
      fp = fopen((log_dir+"/cpu_total_"+processor_name+".txt").c_str(),"w");
      for(auto c:cpus)
        fprintf(fp, "%.2f\n", c);
      fclose(fp);
      mtx.unlock();
      ROS_INFO("Wrote logs for node log_cpu_total at %s !", processor_name.c_str());
      ros::shutdown();
    }
  }
  return 0;
