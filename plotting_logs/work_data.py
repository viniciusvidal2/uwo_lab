...
def work_cpu(cpu, arch):
    ...
    return np.max(cpu.total_edge), np.max(cpu.total_fog)
...
def work_ram(ram, arch):
    ...
    ram_fastlio = ram.fastlio_node / total_ram[0]
    ram_fusecolor = ram.project_image_pointcloud_node / total_ram[1]
    ram_scancontext = ram.scan_context_node / total_ram[2]
    ram_mesh = ram.final_mesh_server_node / total_ram[3]
    ...
    big_size = np.max(np.asarray([len(ram_fastlio), len(ram_fusecolor), len(ram_scancontext), len(ram_mesh)]))
    ram_edge = np.zeros(shape=big_size)
    ram_fog = np.zeros(shape=big_size)
    if arch == 0:
        ram_fog[0:len(ram_fastlio)] += ram_fastlio
        ram_fog[0:len(ram_fusecolor)] += ram_fusecolor
        ram_fog[0:len(ram_scancontext)] += ram_scancontext
        ram_fog[0:len(ram_mesh)] += ram_mesh
    elif arch == 1:
        ram_edge[0:len(ram_fastlio)] += ram_fastlio
        ram_fog[0:len(ram_fusecolor)] += ram_fusecolor
        ram_fog[0:len(ram_scancontext)] += ram_scancontext
        ram_fog[0:len(ram_mesh)] += ram_mesh
    elif arch == 2:
        ram_edge[0:len(ram_fastlio)] += ram_fastlio
        ram_edge[0:len(ram_fusecolor)] += ram_fusecolor
        ram_fog[0:len(ram_scancontext)] += ram_scancontext
        ram_fog[0:len(ram_mesh)] += ram_mesh
    elif arch == 3:
        ram_edge[0:len(ram_fastlio)] += ram_fastlio
        ram_edge[0:len(ram_fusecolor)] += ram_fusecolor
        ram_edge[0:len(ram_scancontext)] += ram_scancontext
        ram_fog[0:len(ram_mesh)] += ram_mesh
